...
class GlobalModelDataset(Dataset):
    ...
    def __getitem__(self,index):
        input_item = self.input_list[index]
        output_item = self.output_list[index]

        return torch.from_numpy(input_item), torch.from_numpy(output_item)
...
class Attention_Net(nn.Module):
    def __init__(self,item_list):
        ...
        self.key_matrix = Variable(torch.randn(self.query_dim,self.key_dim), requires_grad = True)
        self.value_matrix = Variable(torch.randn(self.key_dim,self.feature_dim), requires_grad = True)
        ...
        for i in range(1,self.item_number):
            for j in range(self.item_number - i):
                if int(x[j]) == 1 and int(x[j+i]) == 1:
                    ...
                    mask.append(float(0))
        mask = Variable(torch.from_numpy(np.array(mask)).float(), requires_grad = False)
        #print(mask)
        return mask
...
if __name__ == '__main__':
    ...
    #dataset.print_info("Output")
    #print(dataset.__len__())
    
    #print(dataset.item_index_dic)
